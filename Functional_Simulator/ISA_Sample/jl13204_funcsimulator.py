...
class RegisterFile(object):
    ...
    def Write(self, idx, val):
        ...
        if self.name == "SRF" and len(val) != 1:
            raise ValueError(f"Invalid value length for register with length {self.vec_length}")
        if self.name == "VRF" and len(val) != self.vec_length:
            raise ValueError(f"Invalid value length for register with length {self.vec_length}")
    ...
    def run(self):
        while(True):
                ...
                if opcode == 'ADDVS':
                    ...
                    for x in range(self.VLR[0]):
                        if vrs2[x] == 0:
                            ##raise ZeroDivisionError("Division by zero encountered")
                            pass                         
                        elif self.VMR[x] == 1: vrd[x] = [vrs1[x] // rs2[x]]
                self.RFs["VRF"].Write(int(matches[0]),vrd)
            ...
            if opcode == 'POP':
                ...
                addr = self.RFs["SRF"].Read(int(matches[0]))
                count = [self.VMR.count(1)]    # count 1
                self.RFs["SRF"].Write(addr[0],count)

            # Vector Length Register Operations
            if opcode == 'MTCL':
                matches = re.findall("\d+", line)
                addr = self.RFs["SRF"].Read(int(matches[0]))
                self.VLR = self.RFs["SRF"].Read(addr[0])
            if opcode == 'MFCL':
                addr = self.RFs["SRF"].Read(int(matches[0]))
                matches = re.findall("\d+", line)
                self.RFs["SRF"].Write(addr[0],self.VLR)
            ...
            if opcode == 'SVWS':
                matches = re.findall("\d+", line)
                vrd = self.RFs["VRF"].Read(int(matches[0]))
                addr = self.RFs["SRF"].Read(int(matches[1]))             
                stride = self.RFs["SRF"].Read(int(matches[2])) #stride
                for n in range(self.VLR[0]):  
                    if self.VMR[n] ==1: self.VDMEM.Write(addr[0]+stride[0]*n,vrd[n])
            if opcode == 'LVI':
                matches = re.findall("\d+", line)
                addr = self.RFs["SRF"].Read(int(matches[1]))
                vrs2 = self.RFs["VRF"].Read(int(matches[2]))
            ...
            if opcode == 'SS':
                ...
                matches = re.findall("\d+", line)
                addr = self.RFs["SRF"].Read(int(matches[1]))
                self.SDMEM.Write(addr[0]+Imm,[self.RFs["SRF"].Read(int(matches[0]))])
            ...
            if opcode in ['BEQ','BNE','BGT','BLT','BGE','BLE']:
                rs1 = self.RFs["SRF"].Read(int(matches[0]))
                rs2 = self.RFs["SRF"].Read(int(matches[1]))
                Imm = int(line.split(' ')[3])
